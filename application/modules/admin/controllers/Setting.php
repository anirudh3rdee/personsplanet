...
class Setting extends MY_Controller {
    ...
    public function account_setting(){
        ...
        $data['subview'] = $this->load->view('account_setting', $data, true);
	...
	public function save_log(){
        ...
        if (!$this->upload->do_upload('header_favicon')) {
            ...
            $image_name1 = $data1["image_metadata"]["file_name"];

            $logo_path1 = 'assets/setting_header/'.$image_name1;

            $this->basicModal->insert_setting('header_favicon',$logo_path1);
    ...
    function save_account_setting(){
        if (isset($_POST['new_password'])) {
            ...
            {
                $usersval = $this->basicModal->check('users');
                foreach ($usersval as $users_val) {
                    $userrole = $users_val->u_role;

                    if($userrole == 'admin'){
                       $this->load->model('basicModal');
                      
                      $update = $this->db->query("Update users SET u_password='$new_password' WHERE id='3'");
                       }
                }
            }
            else
    ...
    function save_top_stripe(){
        if (isset($_POST['submit_stripe_content'])) {
            $stripe_link = $_POST['top_stripe_link'];
            $stripe_content = $_POST['stripe_content'];

             $stripeval['stripe_link'] = $stripe_link;
             $stripeval['stripe_content'] = $stripe_content; 

            $stripe_link_val=json_encode($stripeval);

             $this->basicModal->insert_setting('header_top_stripe',$stripe_link_val);
        }
        redirect('admin/setting/header');
    }
