            ...
            <div class="container-fluid">
               ...
               ?>
                ...
                if( !empty( $pageData )){
                    ...
                    //$coursecat = $pageData[0]->coursecat;

                    $array_data = (array) json_decode($pageData[0]->value);

                    $courseDescription = html_entity_decode( trim( $array_data['courseDescription'] ) );
                    $courseFeature = html_entity_decode( trim( $array_data['courseFeature'] ) );
                    $buyNowUrl = trim( $array_data['buyNowUrl'] );
                    $coursePrice = trim( $array_data['coursePrice'] );
                    $old_image = trim( $array_data['featureImage'] );
                    $coursecat = trim( $array_data['coursecat'] );
                }
                            ...
                            <form class="form-horizontal" enctype="multipart/form-data" method="post" action="<?php echo base_url('admin/courses/save_page/'.$pageid);?>">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <label for="coursecat" class="col-sm-3 text-right control-label col-form-label">Course Type</label>

                                        <div class="col-sm-9" style="margin-top:20px">

                                            <select class="form-control" id="coursecat" name="coursecat" value="<?php echo $coursecat;?>">
                                                <option <?php if($coursecat == 'select'){echo("selected");}?>  value="select">Select</option>
                                                <option <?php if($coursecat == 'beginner'){echo("selected");}?>  value="beginner">Beginner</option>
                                                <option <?php if($coursecat == 'advance'){echo("selected");}?>  value="advance">Advance</option>
                                                <option <?php if($coursecat == 'pro_trader'){echo("selected");}?>  value="pro_trader">Pro-Trader</option>
                                            </select>
